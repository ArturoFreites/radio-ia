...
export function DashboardTopbar({
  ...
  return (
    <header className={cn("mb-8 flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between", className)}>
      <div>
        <h1 className="text-2xl font-semibold tracking-tight text-white">Dashboard</h1>
        <p className="mt-1 text-sm text-[color:var(--muted)]">Resumen general de tu radio</p>
      </div>

      <div className="flex flex-wrap items-center gap-3">
        <Button
          className="rounded-2xl border border-white/10 bg-[#09090f] px-5 text-white shadow-[0_0_24px_rgba(255,255,255,0.04)] hover:bg-[#12121a]"
          disabled={!aireToken}
          iconLeft={<Sparkles className="h-4 w-4 text-[color:var(--primary)]" aria-hidden />}
          onClick={abrirCabina}
          size="md"
          variant="secondary"
        >
        ...
        <button
          ...
          className="relative flex h-11 w-11 items-center justify-center rounded-2xl border border-[color:var(--border)] bg-[color:var(--surface)] text-[color:var(--muted)] transition hover:text-white"
        >
        ...
        <div className="relative" ref={profileRef}>
          <button
            ...
            aria-haspopup="menu"
            className="flex items-center gap-3 rounded-2xl border border-[color:var(--border)] bg-[color:var(--surface)] py-1.5 pl-1.5 pr-3 transition hover:bg-[color:var(--surface-soft)]"
          ...
          {profileOpen ? (
            <div
              role="menu"
              className="absolute right-0 top-[calc(100%+0.5rem)] z-50 min-w-[12rem] overflow-hidden rounded-2xl border border-[color:var(--border)] bg-[color:var(--surface)] py-1 shadow-[var(--shadow-dropdown)]"
            >
