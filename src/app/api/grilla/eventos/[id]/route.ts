...
export async function PATCH(request: NextRequest, { params }: { params: Promise<{ id: string }> }): Promise<NextResponse> {
  ...
  const parsed = patchEventoSchema.safeParse(body);
  if (!parsed.success) {
    return NextResponse.json({ error: parsed.error.flatten() }, { status: 400 });
  }
  const raw = parsed.data;

  const evento = await prisma.eventoGrilla.findFirst({ where: { id, radioId } });
  if (!evento) return NextResponse.json({ error: "No encontrado" }, { status: 404 });

  const patch: Record<string, unknown> = {};
  if (raw.fecha !== undefined) {
    const [y, m, d] = raw.fecha.split("-").map(Number);
    if (!y || !m || !d) {
      return NextResponse.json({ error: "fecha invalida" }, { status: 400 });
    }
    patch.fecha = utcDateFromYmd(y, m, d);
  }
  ...
  if (raw.djHoraActiva !== undefined) {
    ...
    patch.djHoraIntervaloMin = raw.djHoraActiva ? (raw.djHoraIntervaloMin ?? evento.djHoraIntervaloMin) : null;
  ...
  if (raw.djClimaActivo !== undefined) {
    ...
    patch.djClimaIntervaloMin = raw.djClimaActivo ? (raw.djClimaIntervaloMin ?? evento.djClimaIntervaloMin) : null;
  ...
  if (raw.djPublicidadActiva !== undefined) {
    ...
    patch.djPublicidadIntervaloMin = raw.djPublicidadActiva
      ? (raw.djPublicidadIntervaloMin ?? evento.djPublicidadIntervaloMin)
      : null;
  ...
  if (
    ...
    const textoActiva = raw.djTextoActiva ?? evento.djTextoActiva;
    const intervalo =
      raw.djTextoIntervaloMin !== undefined ? raw.djTextoIntervaloMin : evento.djTextoIntervaloMin;
    const contenidoRaw =
      raw.djTextoContenido !== undefined ? raw.djTextoContenido : evento.djTextoContenido;
  ...
  if (raw.playlistId !== undefined || raw.playlistNombre !== undefined) {
    const merged = normalizarPlaylistSlot(
      raw.playlistId !== undefined ? raw.playlistId : evento.playlistId,
      raw.playlistNombre !== undefined ? raw.playlistNombre : evento.playlistNombre,
    );
  ...
  if (raw.voz1Id !== undefined || raw.voz2Id !== undefined) {
    const vocesValidadas = await validarVocesSlotParaRadio(
      radioId,
      raw.voz1Id !== undefined ? raw.voz1Id : evento.voz1Id,
      raw.voz2Id !== undefined ? raw.voz2Id : evento.voz2Id,
    );
  ...
  const updated = await prisma.eventoGrilla.update({
    where: { id },
    data: patch,
...
export async function DELETE(_: Request, { params }: { params: Promise<{ id: string }> }): Promise<NextResponse> {
  ...
  const evento = await prisma.eventoGrilla.findFirst({ where: { id, radioId } });
  if (!evento) return NextResponse.json({ error: "No encontrado" }, { status: 404 });
  await prisma.eventoGrilla.delete({ where: { id } });
